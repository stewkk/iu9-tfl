...
std::int32_t Random(std::vector<double> probs) {
  ...
  static std::mt19937 generator(t1.tv_usec * t1.tv_sec);
...
void RegexGenerator::Regex(std::ostringstream& stream, std::int32_t letters_left) {
  ...
  if (letters_left == 1) {
      Symbol(stream);
      if (const auto rnd_value = Random({0.5, 0.5}); current_star_height_+1 <= star_height_ && rnd_value == 0) {
...
RegexGenerator::RegexGenerator(std::int32_t alphabet_power, std::int32_t star_height, std::int32_t max_letters) : alphabet_power_(alphabet_power),
                                                                                                    star_height_(star_height), max_letters_(max_letters)
{}

...
def main(alphabet, initial_regex):
    ...
    # print(final_states, states, transitions, initial_regex)
    print(len(states))
    print(" ".join(states))
    print(len(final_states), " ".join([str(states.index(final_state)) for final_state in final_states]))
    def getTransitions(l, r):
        res = []
        for el in transitions:
            if el['source'] == l and el['dest'] == r:
                res.append(el['trigger'])
        return res if len(res) != 0 else ['0']

    for l in states:
        for r in states:
            print("".join(getTransitions(l, r)), end=' ')
        print()
